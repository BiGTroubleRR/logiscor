// English dictionary — also the canonical shape every other locale must satisfy (see cs.ts).
// Functions are used wherever a string needs interpolation or pluralization.
export const en = {
  header: {
    appName: 'Logiscor',
    tagline: 'Freight Procurement',
    companyList: 'Company List',
    mapView: 'Map View',
    loading: 'loading…',
    countOfCarriers: (filtered: number, total: number) => `${filtered} of ${total} carriers`,
    countOfManufacturers: (filtered: number, total: number) => `${filtered} of ${total} manufacturers`,
    totalCarriers: (n: number) => `${n} carriers`,
    totalManufacturers: (n: number) => `${n} manufacturers`,
    roleManager: 'Procurement Manager',
    roleStaff: 'Ops Staff',
    signOut: 'Sign out',
    langEnglish: 'EN',
    langCzech: 'CZ',
  },

  crmApp: {
    loadingCarriers: 'Loading carriers...',
  },

  auth: {
    appName: 'Logiscor',
    email: 'Email',
    password: 'Password',
    signIn: 'Sign in',
    createAccount: 'Create account',
    noAccountPrefix: 'No account? ',
    signUp: 'Sign up',
    haveAccountPrefix: 'Already have an account? ',
    checkEmail: 'Check your email to confirm your account, then sign in.',
  },

  filterBar: {
    searchPlaceholder: 'Search name, city, or notes...',
    clearSearch: 'Clear search',
    allTypes: 'All types',
    allCountries: 'All countries',
    allCapabilities: 'All capabilities',
    allTrailerTypes: 'All trailer types',
    allProjects: 'All projects',
    possibleDuplicatesOnly: (n: number) => `Possible duplicates only (${n})`,
    manufacturersOnly: (n: number) => `Manufacturers only (${n})`,
    hasQuoteOnly: (n: number) => `Has a quote for this lane (${n})`,
    clearFilters: 'Clear filters',
    advancedFilters: 'Advanced filters ▾',
    hideAdvancedFilters: 'Advanced filters ▴',
    allNdaStatus: 'Any NDA status',
    ndaReceivedOption: 'NDA received',
    ndaNotReceivedOption: 'NDA not received',
    allStrengthTiers: 'Any strength',
    allEmailQuality: 'Any email quality',
    allRowColors: 'Any color',
    addressImpreciseOnly: 'Address not precise only',
    emailFiltered: (n: number) => `✉ Email Filtered (${n})`,
    exportCsv: '⬇ Export CSV',
    exportXlsx: '⬇ Export XLSX',
    template: '⬇ Template',
    import: '⬆ Import',
    addCompany: '+ Add Company',
  },

  routeSearch: {
    heading: 'Route Search',
    originPlaceholder: 'Origin city...',
    destinationPlaceholder: 'Destination city...',
    corridor: (km: number) => `Corridor ${km} km`,
    searching: 'Searching…',
    search: 'Search',
    clearRoute: 'Clear route',
    corridorHint: 'Broaden the corridor slider to widen the search without re-typing places.',
    errorNoPlace: 'Could not find one of those places. Try a more specific city name.',
    errorNoDirections: 'Exact driving route unavailable — showing a straight-line corridor instead.',
    routeSummary: (distanceKm: number, durationMin: number) => {
      const h = Math.floor(durationMin / 60);
      const m = Math.round(durationMin % 60);
      const duration = h > 0 ? `${h}h ${m}m` : `${m}m`;
      return `${Math.round(distanceKm)} km · ${duration}`;
    },
  },

  table: {
    dup: 'DUP?',
    possiblySameAs: (names: string) => `Possibly same as: ${names}`,
    foundInNotes: 'Found in notes',
    dismiss: 'dismiss',
    notDuplicateDismiss: 'Not a duplicate — dismiss',
    none: 'None',
    dash: '—',
    rowColor: 'Row color',
    deleteCompany: 'Delete company',
    colCompany: 'Company',
    colType: 'Type',
    colCountry: 'Country',
    colCity: 'City',
    colCapabilities: 'Capabilities',
    colStrength: 'Strength',
    colTrailerTypes: 'Trailer Types',
    colFlags: 'Routes Served',
    colLastModified: 'Last Modified',
    colColor: 'Color',
    carriersHeading: (n: number) => `Carriers (${n})`,
    manufacturersHeading: (n: number) => `Manufacturers (${n})`,
    genericEmail: 'Generic email',
    genericEmailHint: 'Generic/role-based address (info@, sales@, ...) — may not reach a real contact.',
    noEmail: 'No email',
    noEmailHint: 'No email on file.',
    incompleteInfo: 'Incomplete info',
    incompleteInfoHint: 'No email, phone, or website on file.',
    addressNotPrecise: 'Address not precise',
    addressNotPreciseHint: 'No city on file — the pin may only be a rough country-level guess.',
    colDistance: 'Distance',
    colDistanceRoute: 'Distance (Origin / Dest.)',
    kmFromOrigin: (km: number, place: string) => `${km} km from "${place}"`,
    kmFromDest: (km: number, place: string) => `${km} km from "${place}"`,
    colQuotedForRoute: 'Quoted Rate',
    quotedRateOn: (rate: number, date: string) => `€${rate.toLocaleString()} — quoted ${date}`,
    quotedRateApprox: '⚠ Not this exact route — same country only. ',
    noMatches: 'No carriers match these filters.',
    paginationShowing: (start: number, end: number, total: number) => `Showing ${start}–${end} of ${total}`,
    paginationPrev: 'Previous',
    paginationNext: 'Next',
    paginationPageOf: (page: number, totalPages: number) => `Page ${page} of ${totalPages}`,
  },

  drawer: {
    duplicateCompany: 'Duplicate company',
    addHub: 'Add Hub',
    openFullProfile: 'Open full profile in new tab',
    deleteCompany: 'Delete company',
    close: 'Close',

    strengthScore: 'Strength Score',
    manuallySetScore: 'Manually set score.',
    routeRelevance: (km: number) => `Route relevance: ${km} km off route.`,
    notYetScored: 'Not yet scored — run a route search, or set a manual score below.',
    rationalePlaceholder: 'Rationale for this score...',
    managerOnlyScore: 'Only Procurement Managers can edit the strength score.',
    saveScore: 'Save Score',

    possibleDuplicateOf: (n: number) => `⚠ Possible duplicate of ${n === 1 ? '1 other entry' : `${n} other entries`}`,
    notADuplicate: 'Not a duplicate',
    duplicateFlagDismissed: 'Duplicate flag dismissed for this company.',
    merge: 'Merge',
    mergeIntoThis: (name: string) => `Merge "${name}" into this company`,
    undo: 'Undo',

    companyTypes: 'Company Types',
    addCompanyTypePlaceholder: 'Add company type...',
    projects: 'Projects',
    addProjectPlaceholder: 'Add to project...',
    noProjectsAssigned: 'Not assigned to any project.',
    countriesServed: 'Countries Served',
    addCountryPlaceholder: 'Add country...',
    details: 'Details',
    edit: 'Edit',
    address: 'Address',
    coordinates: 'Coordinates',
    lastUpdated: 'Last updated',
    website: 'Website',
    phone: 'Phone',
    email: 'Email',
    notRecorded: 'Not recorded',
    dash: '—',
    pendingReview: 'Pending review',
    sourcePrefix: 'Source: ',
    viewSource: 'view source',
    notApplicable: 'n/a',
    name: 'Name',
    type: 'Type',
    country: 'Country',
    selectCountryPlaceholder: 'Select country...',
    city: 'City',
    latitude: 'Latitude',
    longitude: 'Longitude',
    searchAddressLabel: 'Search address to update location',
    searchAddressPlaceholder: 'Type an address, city, or place...',
    currentlyPinnedAt: (lat: number, lng: number) => `Currently pinned at ${lat.toFixed(3)}, ${lng.toFixed(3)}`,
    advancedEditCoordinates: 'Advanced: edit coordinates directly',
    hideAdvancedCoordinates: 'Hide advanced coordinates',
    description: 'Description',
    save: 'Save',
    cancel: 'Cancel',

    trailerTypes: 'Trailer Types',
    noneOnFile: 'None on file.',
    addTrailerTypePlaceholder: 'Add trailer type...',
    add: 'Add',
    remove: 'Remove',

    capabilityTags: 'Capability Tags',
    addCapabilityPlaceholder: 'Add capability...',

    activityLog: 'Activity Log',
    logPlaceholder: 'Log a call, email, meeting or note...',
    addEntry: 'Add Entry',
    loading: 'Loading…',
    dateAuthorSep: ' · ',
    noActivityYet: 'No activity logged yet.',

    ratesReceived: 'Rates Received',
    fromProjectBadge: 'From project',

    tabDetails: 'Details',
    tabRates: 'Rates',
    tabNda: 'NDA',
    tabOther: 'Other info',
    ndaReceivedLabel: 'NDA received',
    ndaReceivedDateLabel: 'Date received',
    ndaNotesLabel: 'Notes',
    ndaNotesPlaceholder: 'Which NDA version, who signed, link to the file...',
    ndaReceivedHint: 'NDA received',
    originPlaceholder: 'Origin',
    destinationPlaceholder: 'Destination',
    transportMode: 'Transport Mode',
    selectTransportMode: 'Select transport mode...',
    loadType: 'Load Type',
    selectLoadType: 'Select load type...',
    containerType: 'Container Type',
    selectContainerType: 'Select container type...',
    vehicleType: 'Vehicle / Body Type',
    selectVehicleType: 'Select vehicle type...',
    capacity: 'Capacity',
    selectCapacity: 'Select capacity...',
    cargoTypeHandling: 'Cargo Type & Handling',
    selectCargoType: 'Select cargo type...',
    specialCargo: 'Special Cargo',
    generalCargo: 'General Cargo',
    hazmatClass: 'Hazmat Class',
    selectHazmatClass: 'Select hazmat class...',
    roadFleetType: 'Road Fleet Type',
    selectFleetType: 'Select fleet type...',
    deliveryScope: 'Delivery Scope',
    selectDeliveryScope: 'Select delivery scope...',
    ratePlaceholder: 'Rate (€)',
    demFtPlaceholder: 'DEM / free time',
    notesPlaceholder: 'Notes (RFQ status, date, link...)',
    addRate: 'Add Rate',
    demFtPrefix: 'DEM/F.T.: ',
    noRatesYet: 'No rates on file yet.',
    deleteRate: 'Delete rate',
    expiresAt: 'Expires',
    expiredBadge: 'Expired',
    editRate: 'Edit rate',
    saveRate: 'Save Rate',
    cancelEdit: 'Cancel',
    otherSpecify: 'Other (specify)',
    specifyPlaceholder: 'Specify...',
  },

  mapView: {
    corridorOnly: 'Showing only companies within the searched corridor, colored by relevance.',
    runSearchHint: 'Run a route search above to score and highlight relevant companies.',
    legendStrong: 'Strong (75+)',
    legendMedium: 'Medium (50-74)',
    legendWeak: 'Weak (<50)',
    legendUnscored: 'Unscored',
    showHubs: 'Show hub duplicates',
    origin: 'Origin',
    destination: 'Destination',
  },

  importBtn: {
    noDataRows: 'No data rows found in that file.',
    couldNotRead: "Could not read that file — make sure it's a .xlsx built from the template.",
    importFailedRetry: 'Import failed — please try again.',
    template: '⬇ Template',
    importAction: '⬆ Import',
    previewHeading: 'Import preview',
    rowsReadyToImport: (n: number) => `${n} ${n === 1 ? 'row' : 'rows'} ready to import.`,
    duplicatesFound: (n: number) =>
      `${n} likely ${n === 1 ? 'duplicate' : 'duplicates'} found — excluded by default. Check any you'd like to import anyway (useful for separate hubs of the same company):`,
    importAnyway: 'Import anyway',
    selectAllDuplicates: 'Select all',
    selectNoneDuplicates: 'Select none',
    rowsWillBeSkipped: (n: number) => `${n} ${n === 1 ? 'row' : 'rows'} will be skipped:`,
    rowPrefix: (row: number) => `Row ${row}: `,
    importCount: (n: number) => `Import ${n} ${n === 1 ? 'company' : 'companies'}`,
    cancel: 'Cancel',
    importing: 'Importing…',
    completeHeading: 'Import complete',
    importedCount: (n: number) => `Imported ${n} ${n === 1 ? 'company' : 'companies'}.`,
    rowsSkippedNote: (n: number) => `${n} row(s) were skipped — see previous step for reasons.`,
    done: 'Done',
    failedHeading: 'Import failed',
    close: 'Close',
  },

  tiers: {
    unscored: 'Unscored',
    strong: 'Strong',
    medium: 'Medium',
    weak: 'Weak',
  },

  companyTypes: {
    carrier: 'Carrier',
    manufacturer: 'Manufacturer',
    port: 'Port',
    warehouse: 'Warehouse',
  },

  activityTypes: {
    Call: 'Call',
    Email: 'Email',
    Meeting: 'Meeting',
    Note: 'Note',
  },

  errors: {
    failedToLoad: 'Failed to load.',
    somethingWentWrong: 'Something went wrong.',
    nameRequired: 'Name is required.',
    latLngMustBeNumbers: 'Latitude and longitude must be numbers.',
    couldNotSaveRetry: 'Could not save — please try again.',
    confirmDeleteCompany: (name: string) => `Delete "${name}"? You can restore it later from the Bin.`,
    confirmMergeCompanies: (loserName: string, survivorName: string) =>
      `Merge "${loserName}" into "${survivorName}"? "${loserName}"'s activity log, rate quotes, and project links move to "${survivorName}", and "${loserName}" is sent to the Bin (you can restore it later, but the merge itself won't undo).`,
    confirmPermanentlyDeleteCompany: (name: string) => `Permanently delete "${name}"? This cannot be undone.`,
    confirmDeleteProject: (name: string) => `Delete project "${name}"? Companies assigned to it will be unassigned. This cannot be undone.`,
    confirmOk: 'OK',
    confirmCancel: 'Cancel',
  },

  bin: {
    tabLabel: 'Bin',
    title: 'Bin',
    subtitle: 'Deleted companies can be restored or permanently removed.',
    empty: 'The Bin is empty.',
    loading: 'Loading…',
    deletedOn: (date: string) => `Deleted ${date}`,
    restore: 'Restore',
    deleteForever: 'Delete forever',
    backToList: 'Back to list',
  },

  profilePage: {
    backLink: '← Back to Logiscor',
    loading: 'Loading…',
    notFound: 'Company not found — it may have been deleted.',
  },

  projects: {
    tabLabel: 'Projects',
    title: 'Projects',
    subtitle: 'Group companies you have cooperated with on a specific project.',
    addProject: '+ Add Project',
    empty: 'No projects yet.',
    colName: 'Name',
    colStatus: 'Status',
    colDates: 'Dates',
    colCompanies: 'Companies',
    deleteProject: 'Delete project',
    details: 'Details',
    name: 'Name',
    status: 'Status',
    startDate: 'Start date',
    endDate: 'End date',
    descriptionLabel: 'Description',
    companies: 'Companies',
    noCompaniesAssigned: 'No companies assigned yet.',
    searchCompaniesPlaceholder: 'Search companies to add...',
    exportPartners: '⬇ Export',
    quotedRate: 'Quoted rate',
    quotedRatePlaceholder: 'e.g. 2500',
    remarks: 'Remarks',
    remarksPlaceholder: 'Notes on this price proposal...',
    noQuoteYet: 'No quote yet',
    addToProject: 'Add to project',
    editQuote: 'Edit quote',
    lowestQuote: 'Lowest',
  },

  projectStatuses: {
    active: 'Active',
    on_hold: 'On Hold',
    completed: 'Completed',
    cancelled: 'Cancelled',
  },

  rfq: {
    modalTitle: 'Request for Quote',
    recipients: 'Recipients',
    subjectLabel: 'Subject',
    bodyLabel: 'Message',
    send: 'Send',
    cancel: 'Cancel',
    noEmailOnFile: 'No email on file',
    sendRfq: 'Send RFQ',
    defaultSubject: 'Request for Quote',
    greeting: 'Dear partner,',
    bodyIntro: 'We would like to request a quote for the following route:',
    bodyIntroGeneric: 'We would like to request a quote for your services.',
    routeLine: (origin: string, dest: string) => `Route: ${origin} → ${dest}`,
    cargoLine: (cargo: string) => `Cargo type: ${cargo}`,
    closing: 'Please let us know your availability and rates at your earliest convenience.\n\nBest regards,',
  },

  importValidation: {
    nameRequired: 'Name is required.',
    invalidType: (raw: string, validTypes: string) => `Invalid type "${raw}" — must be one of ${validTypes}.`,
    latRequired: 'Latitude is required and must be a number between -90 and 90.',
    lngRequired: 'Longitude is required and must be a number between -180 and 180.',
    duplicateSkip: (name: string, reason: string) => `"${name}" skipped — ${reason}.`,
    matchesExisting: 'matches a company already in the CRM',
    duplicateInFile: 'duplicate of another row in this file',
  },
};

export type Dict = typeof en;
