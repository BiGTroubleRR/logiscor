...
export default function CompanyTable() {
  ...
  // Manufacturers are excluded from `sorted` by default (see the manufacturersOnly filter in
  // CrmContext) — when the checkbox flips that, this is the only table shown, and it lists
  // exclusively manufacturers, so the heading should say so instead of always saying "Carriers".
  const heading = filters.manufacturersOnly ? t.table.manufacturersHeading(sorted.length) : t.table.carriersHeading(sorted.length);
    ...
    <div style={{ flex: 1, overflow: 'auto', padding: '16px 20px' }}>
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}>
        <div style={{ padding: '10px 14px', borderBottom: '1px solid #e2e8f0', background: '#f8fafc' }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: '#0f172a' }}>{heading}</span>
        </div>
