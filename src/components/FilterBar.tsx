...
export default function FilterBar() {
  ...
  return (
    <div style={{ flex: '0 0 auto', background: '#fff', borderBottom: '1px solid #e2e8f0' }}>
      <div style={{ padding: '12px 20px', display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
        ...
        <button
          ...
          style={{ ...outlineButtonStyle, background: advancedOpen ? '#f1f5f9' : '#fff', color: '#0f172a', fontWeight: 600 }}
        >
          {advancedOpen ? t.filterBar.hideAdvancedFilters : t.filterBar.advancedFilters}
        </button>
      ...
      {/* Advanced filters — everything narrower or less frequently used than Search/Country/
          Capability lives here, collapsed by default so the common case stays uncluttered. */}
      {advancedOpen && (
        <div
          style={{
            padding: '0 20px 12px',
        ...
        >
          ...
          <select value={filters.ndaStatus} onChange={(e) => setFilters({ ndaStatus: e.target.value as typeof filters.ndaStatus })} style={lightSelectStyle}>
            <option value="all">{t.filterBar.allNdaStatus}</option>
            <option value="received">{t.filterBar.ndaReceivedOption}</option>
            <option value="not_received">{t.filterBar.ndaNotReceivedOption}</option>
          </select>

          <select value={filters.strengthTier} onChange={(e) => setFilters({ strengthTier: e.target.value as typeof filters.strengthTier })} style={lightSelectStyle}>
            <option value="all">{t.filterBar.allStrengthTiers}</option>
            <option value="strong">{t.tiers.strong}</option>
            <option value="medium">{t.tiers.medium}</option>
            <option value="weak">{t.tiers.weak}</option>
            <option value="unscored">{t.tiers.unscored}</option>
          </select>

          <select value={filters.emailQuality} onChange={(e) => setFilters({ emailQuality: e.target.value as typeof filters.emailQuality })} style={lightSelectStyle}>
            <option value="all">{t.filterBar.allEmailQuality}</option>
            <option value="generic">{t.table.genericEmail}</option>
            <option value="missing">{t.table.noEmail}</option>
            <option value="incomplete">{t.table.incompleteInfo}</option>
          </select>

          <select value={filters.labelColor} onChange={(e) => setFilters({ labelColor: e.target.value })} style={lightSelectStyle}>
            <option value="all">{t.filterBar.allRowColors}</option>
            {ROW_COLORS.map((rc) => (
              <option key={rc.hex} value={rc.hex}>
          ...
          {/* Off by default — pure manufacturers are hidden from the list entirely so staff
              aren't nudged into contacting them; this is the only way to see them at all. */}
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#334155', border: '1px solid #cbd5e1', borderRadius: 6, padding: '7px 10px', cursor: 'pointer' }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#334155', border: '1px solid #cbd5e1', borderRadius: 6, padding: '7px 10px', cursor: 'pointer' }}>
            <input type="checkbox" checked={filters.addressImpreciseOnly} onChange={() => setFilters({ addressImpreciseOnly: !filters.addressImpreciseOnly })} />
            {t.filterBar.addressImpreciseOnly}
          </label>

          {/* Only meaningful with a searched lane to have a quote for — hidden the rest of the
              time rather than shown disabled, since there's nothing to explain without route context. */}
          {route.active && (
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#334155', border: '1px solid #cbd5e1', borderRadius: 6, padding: '7px 10px', cursor: 'pointer' }}>
