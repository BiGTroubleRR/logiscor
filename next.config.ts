...
//   - Clerk: auth UI, silent-SSO iframe, avatars
//   - This project's own Supabase instance: REST only (no client-side Realtime — see the note
//     atop supabase/schema.sql for why)
...
const csp = [
  ...
  `script-src 'self' 'unsafe-inline' ${process.env.NODE_ENV !== 'production' ? "'unsafe-eval' " : ''}${CLERK_ORIGINS} ${GOOGLE_MAPS_ORIGINS}`,
  `style-src 'self' 'unsafe-inline'`,
  `img-src 'self' data: blob: https://img.clerk.com ${GOOGLE_MAPS_ORIGINS}`,
  `font-src 'self' data:`,
  `connect-src 'self' ${CLERK_ORIGINS} https://clerk-telemetry.com ${SUPABASE_URL} ${GOOGLE_MAPS_ORIGINS} ${PHOTON_ORIGIN}`,
